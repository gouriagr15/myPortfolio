...
const skills = [
  ...
  },
  {
    ...
    level: 70,
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-12 h-12 text-gray-800">
  ...
  },
  {
    ...
    level: 80,
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-12 h-12 text-blue-800">
...
const Skills = () => {
  return (
    <div className="skills bg-gray-900 h-full flex flex-col items-center justify-center text-center text-white py-20 px-4">
      <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-12">My Skills</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 w-full max-w-6xl">
        {skills.map((skill, index) => (
          <div key={index} className="skill-item bg-gray-800 p-6 rounded-lg shadow-lg">
            <div className="flex items-center justify-center mb-4">
              {skill.icon}
            </div>
            <h2 className="text-xl md:text-2xl font-semibold mb-4">{skill.name}</h2>
